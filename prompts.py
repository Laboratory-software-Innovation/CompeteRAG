...
generate_tuner_schema = {
    ...
    "description": (
        ...
        "Given:\n"
            ...
            "  - `hyperparameter_bank`: the existing setup for this subtype of model\n"   
        ...
        " - In your `tuner.search(...)` call, **never** use `hp.*`.  Instead, pick a single fixed integer for `batch_size` and a single fixed integer for `epochs` by taking, say, the **middle** of the list/range in `hyperparameter_bank.params`:\n"
        "     ```python\n"
        "     batch_size = int(sum(chosen_profile[\"param\"][\"batch_size\"][\"values\"]) \n"
        "                       / len(chosen_profile[\"params\"][\"batch_size\"][\"values\"]))\n"
        "     epochs     = (chosen_profile[\"params\"][\"epochs\"][\"min\"] \n"
        "                   chosen_profile[\"params\"][\"epochs\"][\"max\"]) \n"
        "     tuner.search(\n"
