...
X_cat_train = [split_results[i] for i in range(2, 2 + 2*len(categorical_cols), 2)]
X_cat_val = [split_results[i] for i in range(3, 3 + 2*len(categorical_cols), 2)]
...
test_cat = [test[col].values for col in categorical_cols]
...
Traceback (most recent call last):
  File "/app/test/playground-series-s4e9/playground-series-s4e9_kt_solution.py", line 108, in <module>
    n_features = X_num_train.shape[1] + sum([x.shape[1] for x in X_cat_train])
  File "/app/test/playground-series-s4e9/playground-series-s4e9_kt_solution.py", line 108, in <listcomp>
    n_features = X_num_train.shape[1] + sum([x.shape[1] for x in X_cat_train])
