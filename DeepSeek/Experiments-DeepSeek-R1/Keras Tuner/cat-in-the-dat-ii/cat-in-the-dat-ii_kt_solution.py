...
class MyHyperModel(kt.HyperModel):
    def build(self, hp):
        # Continuous input branch
        continuous_input = Input(shape=(train_continuous.shape[1],), name='continuous')
        
        # Nominal embeddings branch
        embedding_branches = []
        nominal_inputs = []
        for col in nominal_cols:
            input_layer = Input(shape=(1,), name=f'nom_{col}')
            vocab_size = len(label_encoders[col].classes_)
            embedding_dim = hp.Int(f'embed_dim_{col}', 2, 50, step=2)
            embedding = Embedding(vocab_size, embedding_dim)(input_layer)
            embedding = Flatten()(embedding)
            nominal_inputs.append(input_layer)
            embedding_branches.append(embedding)
        
        # Concatenate all features
        merged = concatenate([continuous_input] + embedding_branches)
        
        # Tunable dense layers
        for i in range(hp.Int('num_layers', 1, 5)):
            merged = Dense(units=hp.Int(f'units_{i}', 32, 512, step=32),
                         activation=hp.Choice(f'act_{i}', ['relu', 'tanh', 'elu']))(merged)
            merged = Dropout(hp.Float(f'dropout_{i}', 0.0, 0.5, step=0.1))(merged)
...
tuner = kt.BayesianOptimization(
    ...
    project_name='categorical_challenge'
)
...
tuner.search([train_continuous] + train_nominal, y_train,
