...
class MyHyperModel(kt.HyperModel):
    def build(self, hp):
        ...
        # Embedding layers
        embed_size = hp.Int('embed_size', 4, 16, step=4)
        ...
        for _ in range(hp.Int('layers', 2, 8)):
            x = Dense(hp.Int('units', 64, 1024, step=64), activation='relu')(x)
            x = Dropout(hp.Float('dropout', 0.0, 0.5, step=0.1))(x)
        ...
        model.compile(
            optimizer=Adam(hp.Float('learning_rate', 1e-5, 0.01, sampling='log')),
            loss='huber',
...
tuner.search(X_train, y_train, validation_data=(X_val, y_val), callbacks=[es])
...
predictions = best_model.predict(test_inputs).flatten()
