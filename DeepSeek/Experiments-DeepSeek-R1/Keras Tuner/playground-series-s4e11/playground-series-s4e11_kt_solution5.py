...
class MyHyperModel(kt.HyperModel):
    ...
    def fit(self, hp, model, *args, **kwargs):
        # Remove validation_split to prevent double splitting
        if 'validation_split' in kwargs:
            del kwargs['validation_split']
            
        # Get actual training data size from tuner's split
        x_data = args[0]
        num_train_samples = x_data.shape[0]
...
<Error>
Here is the corrected build method:

Python

# 1. Add the necessary imports at the top of your script
...
class MyHyperModel(kt.HyperModel):
    def build(self, hp):
        ...
        # Instantiate the optimizer with the chosen learning rate
        ...
        # Build the model (this part is fine)
        inputs = Input(shape=(n_features,))
        x = inputs
        for _ in range(layers):
            x = Dense(units, activation=act)(x)
        ...
        # Compile the model with the instantiated optimizer object
