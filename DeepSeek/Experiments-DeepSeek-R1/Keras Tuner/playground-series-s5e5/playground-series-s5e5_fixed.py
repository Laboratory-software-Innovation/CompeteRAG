...
class MyHyperModel(kt.HyperModel):
    def build(self, hp):
        ...
        model.add(Dense(1, activation='relu'))  # Ensure non-negative outputs
        model.compile(
            optimizer=Adam(learning_rate=learning_rate),
            loss='mse',
            metrics=[tf.keras.metrics.RootMeanSquaredError()] # CHANGED to RMSE
        )
        return model
...
# Training with early stopping and time tracking
...
# Get metrics from the best epoch (due to restore_best_weights=True)
