...
# Prepare test predictions - FIX: Drop 'hand' column from test data
test_df = pd.read_csv('test/poker-rule-induction/poker-rule-induction/test.csv.zip')
X_test = scaler.transform(test_df.drop('hand', axis=1).values)  # Fixed line
...
Traceback (most recent call last):
  File "/app/test/poker-rule-induction/poker-rule-induction_fixed.py", line 70, in <module>
    X_test = scaler.transform(test_df.drop('hand', axis=1).values)  # Fixed line
  File "/usr/local/lib/python3.10/dist-packages/pandas/core/frame.py", line 5588, in drop
    return super().drop(
  File "/usr/local/lib/python3.10/dist-packages/pandas/core/generic.py", line 4807, in drop
    obj = obj._drop_axis(labels, axis, level=level, errors=errors)
  File "/usr/local/lib/python3.10/dist-packages/pandas/core/generic.py", line 4849, in _drop_axis
    new_axis = axis.drop(labels, errors=errors)
  File "/usr/local/lib/python3.10/dist-packages/pandas/core/indexes/base.py", line 7098, in drop
